...
defmodule Example do
  ...
  def tupleExample do
    ...
    IO.puts(
      "The total average for #{elem(memberships, 0)}, #{elem(memberships, 1)} and #{elem(memberships, 2)} memberships is #{avg}"
    )
  end
  ...
  def guess do
    ...
    guess = gets("Guess a number between 0 and 9 ") |> String.trim() |> Integer.parse()

    IO.inspect(guess)

    case guess do
      {result, _} ->
        puts("Parse Successful #{result}")
